...
interface ICoreInteractionHandlerReadable {
    ...
    function spotTokenBTC() external view returns (uint64);

    function spotTokenHYPE() external view returns (uint64);

    function spotBTC() external view returns (uint32);
...
contract CoreInteractionViews {
    ...
    /// @notice Prix BTC normalisé en 1e8 (USD 1e8) via l'oracle spot.
    function oraclePxBtc1e8(
        ICoreInteractionHandlerReadable handler
    ) external view returns (uint64) {
        uint32 spotB = handler.spotBTC();
        return _spotOraclePx1e8(handler, spotB);
    }

    /// @notice Equity spot Core (USDC+BTC+HYPE) en USD 1e18, en lisant directement l'état du handler.
    ...
    function equitySpotUsd1e18(
        ...
        uint64 btcTokenId = handler.spotTokenBTC();
        uint64 hypeTokenId = handler.spotTokenHYPE();

        uint256 usdc1e18 = _usdcUsd1e18(l1, handlerAddr, usdcTokenId);
        uint256 btcUsd1e18 = _assetUsd1e18(l1, handler, handlerAddr, btcTokenId, handler.spotBTC());
