...
async function main() {
  const { ethers } = hre;
  console.log("\n🚀 Déploiement ERA_2 sur HyperEVM Testnet\n");
  ...
  // Utiliser le nom qualifié complet pour éviter l'ambiguïté entre STRATEGY_1 et ERA_2
  const L1Read = await ethers.getContractFactory("src/ERA_2/interfaces/L1Read.sol:L1Read");
  ...
  // Paramètres (identique à STRATEGY_1 sauf SPOT_TOKEN1_ID et TOKEN1_TOKEN_ID)
  ...
  const SPOT_TOKEN1_ID = 1137; // Différent de STRATEGY_1 (1054)
  const SPOT_HYPE_ID = 1035;
  const USDC_TOKEN_ID = 0;
  const TOKEN1_TOKEN_ID = 1242; // Différent de STRATEGY_1 (1129)
  ...
  console.log("\n🔧 Déploiement CoreHandlerLogicLib + CoreInteractionHandler (ERA_2)...");

  // Utiliser les contrats de ERA_2 avec noms qualifiés complets
  const CoreHandlerLogicLib = await ethers.getContractFactory("src/ERA_2/CoreHandlerLogicLib.sol:CoreHandlerLogicLib");
  ...
  const CoreInteractionHandler = await ethers.getContractFactory("src/ERA_2/CoreInteractionHandler.sol:CoreInteractionHandler", {
    libraries: {
  ...
  await send(handler.setSpotIds(SPOT_TOKEN1_ID, SPOT_HYPE_ID, { gasPrice }));
  await send(handler.setSpotTokenIds(USDC_TOKEN_ID, TOKEN1_TOKEN_ID, HYPE_TOKEN_ID, { gasPrice }));
  ...
  console.log("\n🔧 Déploiement VaultContract (ERA_2)...");
  const Vault = await ethers.getContractFactory("src/ERA_2/VaultContract.sol:VaultContract");
  ...
  console.log("\n🔧 Déploiement CoreInteractionViews (ERA_2)...");
  const CoreInteractionViews = await ethers.getContractFactory("src/ERA_2/CoreInteractionViews.sol:CoreInteractionViews");
  ...
  console.log("\n📋 Adresses (HyperEVM Testnet - ERA_2):");
  console.log("L1Read:", await l1.getAddress());
  console.log("CoreHandlerLogicLib:", await coreHandlerLogicLib.getAddress());
  ...
  console.log("SPOT_TOKEN1_ID:", SPOT_TOKEN1_ID);
  console.log("SPOT_HYPE_ID:", SPOT_HYPE_ID);
  console.log("TOKEN1_TOKEN_ID:", TOKEN1_TOKEN_ID);
  console.log("HYPE_TOKEN_ID:", HYPE_TOKEN_ID);
  console.log("USDC_TOKEN_ID:", USDC_TOKEN_ID);
  console.log("\n🎉 Déploiement ERA_2 terminé sur testnet.");
}
