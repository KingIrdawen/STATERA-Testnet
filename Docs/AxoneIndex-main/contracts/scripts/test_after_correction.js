...
async function main() {
  ...
  // Avec la correction: spotBalanceInWei() retourne total directement (déjà en weiDecimals)
  const hypeBalWei = BigInt(hypeBalance.total.toString()); // Pas de conversion
  const hypeHuman = Number(hypeBalWei) / (10 ** Number(hypeInfo.weiDecimals));
  
  console.log(`\n📊 APRÈS CORRECTION:`);
  console.log(`  Balance wei: ${hypeBalWei.toString()}`);
  console.log(`  Balance humaine: ${hypeHuman} HYPE`);
  console.log(`  ✅ Vérification: ${hypeHuman} HYPE (attendu: ~0.616986 HYPE)`);
  ...
  console.log(`  ✅ Vérification: ${hypeHuman} HYPE × ${ethers.formatUnits(pxHype1e8, 8)} USD = ${hypeHuman * Number(ethers.formatUnits(pxHype1e8, 8))} USD`);
  ...
  const usdcBalWei = BigInt(usdcBalance.total.toString()); // USDC aussi déjà en weiDecimals
  ...
  // Convertir en 1e8
  const size1e8 = sizeSz * (10n ** BigInt(8 - Number(hypeInfo.szDecimals)));
  
  console.log(`\n📊 TAILLE CALCULÉE:`);
  console.log(`  sizeSz: ${sizeSz.toString()} (szDecimals)`);
  console.log(`  size1e8: ${size1e8.toString()}`);
  console.log(`  Taille humaine: ${Number(size1e8) / 1e8} HYPE`);

  // Comparer avec l'ordre observé (qui était 1e6 trop grand)
  const observedHypeSizeSz = 49195531n;
  const observedHypeSize1e8 = observedHypeSizeSz * (10n ** BigInt(8 - Number(hypeInfo.szDecimals)));
  
  console.log(`\n📊 COMPARAISON:`);
  console.log(`  Taille calculée: ${size1e8.toString()} (1e8) = ${Number(size1e8) / 1e8} HYPE`);
  console.log(`  Taille observée (ancienne, 1e6 trop grande): ${observedHypeSize1e8.toString()} (1e8) = ${Number(observedHypeSize1e8) / 1e8} HYPE`);
  console.log(`  Ratio: ${Number(size1e8) / Number(observedHypeSize1e8)}`);
  console.log(`  ✅ Si ratio ≈ 1e-6, la correction est correcte !`);
