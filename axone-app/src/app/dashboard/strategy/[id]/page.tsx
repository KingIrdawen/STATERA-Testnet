...
export default function StrategyStatsPage() {
  ...
  const tokenIdContracts = handlerAddress && contracts?.handler
    ? [
        {
          ...contracts.handler,
  ...
  const hypeTokenId = tokenIdsData?.[0]?.result as bigint | undefined;
  const token1TokenId = tokenIdsData?.[1]?.result as bigint | undefined;
  ...
  if (tokenData && Array.isArray(tokenData) && tokenData.length >= 4 && strategyData.oracleHypeUsd !== undefined && strategyData.oracleToken1Usd !== undefined) {
    try {
      // HYPE
