// axone-app/src/hooks/usePpsHistory.ts
import { useState, useEffect } from 'react';

export interface PpsEntry {
  timestamp: number; // milliseconds
  pps: string; // PPS value as string
  blockNumber?: number;
  txHash?: string;
}
...
export function usePpsHistory(vaultAddress: string | undefined, limit?: number) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const url = new URL(`/api/strategies/${encodeURIComponent(vaultAddress)}/pps`, window.location.origin);
        if (limit) {
        ...
        if (!res.ok) {
          ...
          throw new Error(body.error || 'Error loading PPS history');
        }
